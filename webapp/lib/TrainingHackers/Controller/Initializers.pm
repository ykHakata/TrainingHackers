...
sub index {
    ...
    my $data = [
        ...
        {
            question => '以下のプログラムはとあるサイトにおけるログイン処理の一部である。$uidにはユーザー名、$passwordにはパスワードが入力されているとする。本来であればユーザー名とパスワードが一致しないとログインは出来ないが$uidにある入力を行うとパスワードを入力しなくてもログインができてしまう。その入力を答えよ。入力文字数は全部で12文字とする。なおこのサイトではkusakabeというユーザー名が存在していることは確認されている。',
            answer => 'kusakabe\'-- ',
            score => 10,
            level => 10,
